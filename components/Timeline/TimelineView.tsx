...
const TimelineView: React.FC<TimelineViewProps> = ({ 
  ...
  // Helper for accurate local "Today" string
  const getTodayStr = () => {
    const now = new Date();
  ...
  // Safeguard for profile name to prevent 'split' error
  ...
  const filteredItems = tasks
    ...
    .sort((a, b) => {
      // Sort by date first if in Upcoming view
                ...
                >
                  <div className="flex flex-col">
                    <div className="flex justify-between items-start">
                      <h3 className={`text-[17px] font-bold leading-tight tracking-tight ${item.completed ? 'line-through text-slate-400' : 'text-slate-800 dark:text-white'}`}>
                        {item.title}
                      </h3>
                    ...
                    {item.details && (
                      <p className="text-[12px] text-blue-600 dark:text-blue-400 font-bold mt-0.5 tracking-tight opacity-90">
                        {item.details}
                      </p>
                    )}

                    {item.isBigNote && item.description && (
                      <p className="text-[13px] text-slate-500 dark:text-slate-400 mt-1 line-clamp-2 leading-relaxed font-medium">
                        {item.description}
                      </p>
                    )}
...
const StatusCard = ({ count, label, color = "text-slate-800 dark:text-white", isActive = false, onClick }: { count: number, label: string, color?: string, isActive?: boolean, onClick?: () => void }) => (
  ...
  >
    <span className={`text-[22px] font-black ${color}`}>{count}</span>
